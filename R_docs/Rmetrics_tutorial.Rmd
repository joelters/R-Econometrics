...
```
The regression approach is useful since it allows to adjust for covariates. This means adding other covariates $X_i$ as regressors. Since $D_i$ is randomly assigned, $X_i$ will be independent of $D_i$ and hence will not affect the parameter associated to $D_i$, also, if $X_i$ has explanatory power over $Y_i$ it will increase the precision of our ATE estimator by reducing the variance of the residuals. For instance, following Lalonde (1986) and Dehejia & Wahba (1999) we can adjust for age, age squared, education, race and high school degree. 
...
To be able to identify treatment effects we still need to assume selection on observables: $A1': (Y(1)_i, Y(0)_i) \perp D_i | X_i$. That is, the distribution of the potential outcomes now can vary for individuals with different treatments as long as they have different covariates, however, for those with the same covariates, the potential outcomes still have to be independent from treatment. Let us define and ATE which might depend on $X_i$ and denote it as $ATE(X_i)$ (also called CATE)
...
If the ATE is constant across the support of $X_i$, then $ATE = ATE(X_i)$. One case in which this is true is under the assumption that $\mathbb{E}[Y_i | D_i, X_i] = \delta_0 + \tau D_i + \delta_1' X_i$. Under this linearity assumption, the ATE can be estimated by OLS
...
Y <- obs_df$re78
D <- obs_df$treat
X <- select(obs_df, c("age", "education", "black", "hispanic", "nodegree"))
X$agesq <- X$age^2
...
````{r}
obs_dftrim <- filter(obs_df, pscore > 0.25)
...
\frac{D_i Y_i}{G(X_i)} - \frac{(1-D_i)Y_i}{1 - G(X_i)} - \tau \\
X_i(D_i - G(X_i'\beta))
\end{pmatrix}
$$
...
$$
\hat{V}_1 = \biggl(n^{-1}\sum_{i=1}^n \dot{q}(Z_i,\theta_0)\biggr)^{-1} \biggl(n^{-1}\sum_{i=1}^nq(Z_i, \theta_0)^2 \biggr) \biggl(n^{-1}\sum_{i=1}^n \dot{q}(Z_i,\theta_0)\biggr)^{-1}
$$
...
-1 & -X_i'\biggl(\frac{D_iY_i}{G(X_i'\beta)} - \frac{(1-D_i)Y_iG(X_i'\beta)(1-G(X_i'\beta))}{(1-G(X_i' \beta))^2}\biggr) \\
...
X <- as.matrix(select(obs_df,c(const,age,agesq,education,educationsq,married,
...
for (i in 1:nrow(X)){
  aux <-  matrix(c(D[i]*Y[i]/PS[i] - (1-D[i])*Y[i]/(1-PS[i]) - ipw1,
                    X[i,]*(D[i] - PS[i])),k+1,1)
  A <- A + aux%*%t(aux)
  B12 <- B12 - t(X[i,])*((D[i]*Y[i])/PS[i] + ((1-D[i])*Y[i]*PS[i]*(1-PS[i]))/(1-PS[i])^2)
...
$$
To estimate it let first split the data. We pick $L=2$ to ensure there are some treated observations in each split. We randomly allocate half of the treated and half of the control in one split(df1) and the rest in the other split (df2)

```{r}
ind1 <- sample(1:sum(obs_df$treat), floor(sum(obs_df$treat)/2))
treat1 <- filter(obs_df, treat == 1)[ind1,]
treat2 <- filter(obs_df, treat == 1)[-ind1,]
ind2 <- sample(1:sum(1 - obs_df$treat), floor(sum(1 -obs_df$treat)/2))
contr1 <- filter(obs_df, treat == 0)[ind2,]
contr2 <- filter(obs_df, treat == 0)[-ind2,]

df1 <- rbind(treat1,contr1)
df2 <- rbind(treat1,contr2)
```
